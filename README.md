...
- 圣遗物评分：`喵喵圣遗物评分 <UID> [角色]`（读取本地 Yunzai `miao-plugin` 的角色评分权重，并支持绝缘4/西风/薙草高精等常见规则）
- 伤害估算：`喵喵伤害计算 <UID> [角色]`
...
当前版本为 **功能语义迁移（v0.11.0）**，重点迁移命令形态、UID 绑定、面板管理入口、配置管理、更新日志、基础权限控制、面板数据源、图片模板、角色别名、圣遗物评分与伤害估算。
...
圣遗物评分已迁移本地 Yunzai `miao-plugin` 的常用角色权重和部分动态规则；伤害估算仍为首版通用算法，后续会继续逐角色对齐 miao-plugin 计算模板。
