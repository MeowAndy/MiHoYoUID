...
def _base_damage(char: Dict[str, Any], scale: float, *, base_key: str = "atk", react: float = 1.0, extra_bonus: float = 0.0, extra_cdmg: float = 0.0, extra_cpct: float = 0.0, break_mul: float = 1.0) -> Tuple[float, float]:
    ...
    crit = min(max(_pct_attr(props, ("暴击率", "cpct", "crit_rate", "critRate"), 5) + extra_cpct, 0), 1)
    cdmg = max(_pct_attr(props, ("暴击伤害", "cdmg", "crit_dmg", "critDamage"), 50) + extra_cdmg, 0)
    dmg_bonus = max(_pct_attr(props, ("伤害加成", "元素伤害加成", "dmg", "damage", "element_dmg"), 0) + extra_bonus, 0)
    no_crit = base * scale * (1 + dmg_bonus) * react * break_mul
...
def _char_damage_details(char: Dict[str, Any], game: str) -> Tuple[str, List[Dict[str, float | str]], List[str]]:
    ...
    if game != "sr" and "胡桃" in name:
        ...
        d1, a1 = _base_damage(tmp, ca_scale, extra_bonus=0.33)
        d2, a2 = _base_damage(tmp, ca_scale, react=1.5, extra_bonus=0.33)
        d3, a3 = _base_damage(tmp, q_scale, extra_bonus=0.33)
    ...
    if game != "sr" and "那维莱特" in name:
        ...
        for title, scale in (("重击伤害", a2_multi * (0.072 + a_lv * 0.0068)), ("E伤害", e), ("Q释放伤害", q), ("满水滴一轮重击总伤害", ca)):
            d, a = _base_damage(tmp, scale, base_key="hp", extra_bonus=0.30, extra_cdmg=extra_cdmg)
    ...
    if game != "sr" and ("雷电" in name or "雷神" in name):
        ...
        for title, scale in (("零愿力Q首刀", 4.01 + q_lv * 0.37), ("满愿力Q首刀", first), ("满愿力Q后重击", ca)):
            d, a = _base_damage(char, scale, extra_bonus=extra_bonus)
            rows.append(_fmt_damage(title, d, a))
    ...
    if game == "sr" and "黄泉" in name:
        ...
        for title, scale in (("普攻伤害", 1.0 + a_lv * 0.10), ("战技伤害·主目标", 1.6 + e_lv * 0.16), ("终结技伤害·对单", 3.7 + q_lv * 0.36 + 1.5)):
            d, a = _base_damage(char, scale * extra_mul * extra_kx, extra_bonus=extra_bonus, extra_cpct=extra_cpct)
            rows.append(_fmt_damage(title, d, a))
    ...
    if game == "sr" and "流萤" in name:
        ...
        for title, mul in (("强化战技伤害", 1.0), ("破韧后战技主目标伤害", break_bonus), ("破韧后战技副目标伤害", break_bonus * 0.56)):
            d, a = _base_damage(char, scale * mul * ignore, break_mul=1 + max(stance, 0) / 500)
            rows.append(_fmt_damage(title, d, a))
    ...
    if game == "sr" and "镜流" in name:
        ...
        for title, scale in (("普攻伤害", 1.0 + a_lv * 0.09), ("战技伤害", 2.0 + e_lv * 0.20), ("转魄状态·战技伤害(扩散)", 3.0 + e_lv * 0.30), ("转魄状态·终结技伤害(扩散)", 4.0 + q_lv * 0.38)):
            ...
            d, a = _base_damage(tmp, scale, extra_bonus=bonus, extra_cpct=extra_cpct, extra_cdmg=extra_cdmg)
            rows.append(_fmt_damage(title, d, a))
    ...
    ], [f"核心属性：{dmg.get('core_type', '攻击')}；该角色暂未接入独立 calc.js 规则"]
...
def collect_damage_rows(result: PanelResult, character_query: str = "") -> Tuple[List[Dict[str, Any]], str]:
    ...
    for char in characters[:8]:
        template, details, notes = _char_damage_details(char, result.game)
        rows.append({
            "name": _char_name_for_game(char, result.game),
            "template": template,
            "details": details,
            "notes": notes,
            "level": char.get("level") or "?",
            "constellation": char.get("constellation") or 0,
            "skills": char.get("skill_levels") or [],
        })
    return rows, ""
